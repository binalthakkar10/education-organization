...
<script type="text/javascript">
    ...
    errorClass: 'errMsg',
            rules: {
                summercamp_class_code: "required",
                location_code: "required",
                summercamp_code: "required",
                  single_pay_amt:{
                            number: true
                        },
            ...
            messages: {
                course_code: "Please select course type",
                location_code: "Please select course type",
                class_code: "Please enter course type"
            }
        });
    });
</script>
        ...
        <div class="element">
            ...
            <select name="summercamp_class_code" id="course_code" class="err">
                ...
                <?php foreach ($campcode as $courseCode) { ?>
                    <option value="<?php echo $courseCode['camp_code'] ?>" <?php
                            if ($courseCode['id'] == $fInfoData["id"]) {
                                echo"selected";
                            }
                            ?>><?php echo ucfirst($courseCode['camp_code']); ?></option>
<?php } ?>
            </select>
        </div>                   
        <div class="element">
            ...
            <select name="location_code" id="location_code" class="err">
                <option value="">-- select Location Code--</option>
                    ...
                    <option value="<?php echo $loc['loc_code'] ?>" <?php
                        if ($loc['id'] == $fInfoData["id"]) {
                            echo"selected";
                        }
                        ?>><?php echo ucfirst($loc['loc_code']); ?></option>      
<?php } ?>
            </select>            
        </div>

        <div class="element">
            <label> Summer Camp Class Code<span class="red">(required)</span><span class="errMsg"></span></label>
...
<!--            <select name="days_of_week[]" size="8" multiple >
...
<option value="sunday">Sunday</option>
            ...
            <textarea type="textarea"  rows="4" cols="35"  class="field size1"  name="day_of_week"  id="day_of_week" size="5" value="<?php echo isset($InfoData['day_of_week']) ? $InfoData['day_of_week'] : '' ?>">
</textarea> 
            <!--<input class="field size1"  name="day_of_week" type="text"  id="day_of_week" size="5" value="<?php echo isset($InfoData['day_of_week']) ? $InfoData['day_of_week'] : '' ?>" />-->
        </div>
        <div class="element">
...
<?php } ?>           
       ...
       <?php $teachers=$InfoData['teacher_assigned'];?>
             <div class="element">                       
            <label>Teacher Assigned</label>       
            <select name="teacher_assigned" id="teacher_assigned" class="err">

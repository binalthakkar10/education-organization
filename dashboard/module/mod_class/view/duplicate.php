...
if (isset($_POST['id'])) {
    $userids = $_POST['id'];
        $obj->admin_duplicateClass($_POST);
        flash($pagetitle . " is successfully duplicated");
    ...
    $(document).ready(function() {
        ...
        room: "required",
                ...
                bannerimage: {
                  ...
                  class_roster: {accept: "pdf|doc"},
                  attendance_sheet: {accept: "pdf|doc"},   
                  invoice_amount: {number: true}, 
                  recieved_amount: {number: true},          
...
<div class="full_w">
    ...
    <form action="" method="post" name="itffrminput" id="itffrminput" enctype="multipart/form-data">
        ...
        <div class="element">
            ...
            <input class="field size1"  name="code" type="text"  id="class_code" size="35" value="<?php echo isset($InfoData['code']) ? $InfoData['code'] : '' ?>" />
        </div>
        ...
        <div class="element">
            ...
            <select name="course_id" id="course_code" class="err">
                <option value="">-- select Course Code--</option>
                <?php foreach ($course as $courseCode) { ?>
                    <option value="<?php echo $courseCode['id'] ?>"><?php echo ucfirst($courseCode['code']) . ' - ' . $courseCode['name']; ?></option>
                        <?php } ?>
            </select>
        </div>   
...
//print_r($dropdownClass);
        ...
        <div class="element">
            <label>Notes for Parents <span class="red">(required)</span></label>
            <textarea class="field size1"  name="notes"   id="notes"  rows="5" cols="30" /><?php echo isset($InfoData['notes']) ? $InfoData['notes'] : '' ?></textarea>
        </div>        
        
        <div class="element">
            <label>Notes for Teacher <span class="red">(required)</span></label>
            <textarea class="field size1"  name="teacher_notes"   id="teacher_notes"  rows="5" cols="30" /><?php echo isset($InfoData['teacher_notes']) ? $InfoData['teacher_notes'] : '' ?></textarea>
        </div> 
      <div class="element registration_type">
        ...
        <div id="external" <?php if ($InfoData['registration_type'] != 'external') { ?>style="display:none" <?php } ?>>
        <div class="element">
         ...
         <div class="element">
         <label> Class Roster <span class="red"></span></label>
        <div id="FileUpload">
                <input type="file" size="24" id="class_roster" name="class_roster" class="BrowserHidden"  />
                <div id="BrowserVisible">
                    <?php if ($InfoData['class_roster']) { ?>
                        <div class="display"> <a href="<?php echo SITEURL . "/itf_public/class_roster/" . $InfoData['class_roster'] ?>" target="blank" ><?php echo $InfoData['class_roster'] ?></a> </div>
                    <?php } //$InfoFiledata?>
                </div>
            </div>
            </div>
         <div class="element">    
        <label> Attendance Sheet <span class="red"></span></label>
        <div id="FileUpload">
                <input type="file" size="24" id="attendance_sheet" name="attendance_sheet" class="BrowserHidden"  />
                <div id="BrowserVisible">
                    <?php if ($InfoData['attendance_sheet']) { ?>
                        <div class="display"> <a href="<?php echo SITEURL . "/itf_public/attendance_sheet/" . $InfoData['attendance_sheet'] ?>" target="blank" ><?php echo $InfoData['attendance_sheet'] ?></a> </div>
                    <?php } //$InfoFiledata?>
                </div>
            </div>
            </div>
            <div class="element">
            <label>Date Invoice Sent</label>
            <input class="field size1 tcal" autocomplete="off"  name="invoice_date" type="text"  id="invoice_date" size="35" value="<?php echo isset($InfoData['invoice_date']) ? date('m/d/Y', strtotime($InfoData['invoice_date'])) : '' ?>"  readonly/>
        </div>
        
        <div class="element">
            <label>Invoice Amount<span class="red"></span></label>
            <input class="field size1"  name="invoice_amount" type="text" id="invoice_amount" size="35" value="<?php echo isset($InfoData['invoice_amount']) ? $InfoData['invoice_amount'] : '' ?>" />
        </div>
        
        <div class="element">
            <label>Date Payment Received</label>
            <input class="field size1 tcal" autocomplete="off"  name="received_date" type="text"  id="received_date" size="35" value="<?php echo isset($InfoData['received_date']) ? date('m/d/Y', strtotime($InfoData['received_date'])) : '' ?>"  readonly/>
        </div>
        
         <div class="element">
            <label>Amount Received<span class="red"></span></label>
            <input class="field size1"  name="recieved_amount" type="text" id="recieved_amount" size="35" value="<?php echo isset($InfoData['recieved_amount']) ? $InfoData['recieved_amount'] : '' ?>" />
        </div>
        
        <div class="element">
            <label>Notes for Invoice <span class="red">(required)</span></label>
            <textarea class="field size1"  name="invoice_notes"   id="invoice_notes"  rows="5" cols="30" /><?php echo isset($InfoData['invoice_notes']) ? $InfoData['invoice_notes'] : '' ?></textarea>
        </div> 
       ...
       <!--<input class="field size1"  name="teacher_assigned" type="text"  id="teacher_assigned" size="35" value="<?php echo isset($InfoData['teacher_assigned']) ? $InfoData['teacher_assigned'] : '' ?>" />-->
          ...
          <div class="element checkinternal" <?php if ($InfoData['registration_type'] == 'external') { ?>style="display:none" <?php } ?>>

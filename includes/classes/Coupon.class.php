...
class Coupon {
    ...
    function admin_addCoupon($datas) {
        ...
        $this->dbcon->Insert('itf_coupon', $datas);
    }

    function admin_updateCoupon($datas) {
    	if(!empty($_SESSION['LoginInfo']['org_id'])&& isset($_SESSION['LoginInfo']['org_id'])){
    		$org_id = $_SESSION['LoginInfo']['org_id'];
    		$datas['org_id'] = $org_id;
    	}
        ...
        $endDate = date('Y-m-d', strtotime($datas['end_date']));
        $datas['end_date'] = $endDate;
        $this->dbcon->Modify('itf_coupon', $datas, $condition);
    }

    function Coupon_deleteAdmin($Id) {
        $sql = "delete from itf_coupon where id in(" . $Id . ")";
        $this->dbcon->Query($sql);
        return $this->dbcon->querycounter;
    }
	function ClassCoupon(){
		if(!empty($_SESSION['LoginInfo']['org_id'])&& isset($_SESSION['LoginInfo']['org_id'])){
			$org_id = $_SESSION['LoginInfo']['org_id'];
			$sql = "select * from itf_coupon where scope= 'local' and type= 'C' and org_id=".$org_id;
		}else{
			$sql = "select * from itf_coupon where scope= 'local' and type= 'C'";
		}	
		return $this->dbcon->FetchAllResults($sql);
	}
	
	function CouponClassData($data){
			$sql = "select * from itf_coupon_class where class_id='".$data."' ";		
		return $this->dbcon->FetchAllResults($sql);
	
	}
	
	function TournamentCoupon(){
		 $sql = "select * from itf_coupon where scope= 'local' and type= 'T' ";		
		return $this->dbcon->FetchAllResults($sql);
	}
	
	function CouponTournamentData($data){
			$sql = "select * from itf_coupon_tournament where tournament_id	='".$data."' ";		
		return $this->dbcon->FetchAllResults($sql);
	
	}
	
	function SummerCoupon(){
		 $sql = "select * from itf_coupon where scope= 'local' and type= 'S' ";		
		return $this->dbcon->FetchAllResults($sql);
	}
	
	function CouponSummerData($data){
			$sql = "select * from itf_coupon_summercamp where summercamp_id='".$data."' ";		
		return $this->dbcon->FetchAllResults($sql);
...
///////////////
    ...
    function showAllCoupons() {
    	if(!empty($_SESSION['LoginInfo']['org_id'])&& isset($_SESSION['LoginInfo']['org_id'])){
    		$org_id = $_SESSION['LoginInfo']['org_id'];
    		$sql = "select * from itf_coupon where  org_id='".$org_id."'  order by id desc";
    	}else{
    		$sql = "select * from itf_coupon order by id desc";
    	}
        return $this->dbcon->FetchAllResults($sql);
    }
    ...
    function CouponDetail($UsId) {
        $sql = "select C.* from itf_coupon C where C.id='" . $UsId . "'";
